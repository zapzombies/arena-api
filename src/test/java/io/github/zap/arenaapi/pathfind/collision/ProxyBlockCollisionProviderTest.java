...
class ProxyBlockCollisionProviderTest {
    ...
    private BlockCollisionView mockBlockAt(int x, int y, int z, List<BoundingBox> voxelShapes, boolean shouldOverlap) {
        ...
        if(voxelShapes == fullBlock || voxelShapes == stairs) {
            Mockito.when(mockVoxelShapeWrapper.boundingBox()).thenReturn(fullBlockBounds);
        }
        else if(voxelShapes == tinyBlock) {
            Mockito.when(mockVoxelShapeWrapper.boundingBox()).thenReturn(tinyBlockBounds);
        }
        else {
            throw new IllegalArgumentException("shape list not supported");
        }
    ...
    private void testWalkDirection(BoundingBox agentBounds, List<BlockCollisionView> collisions, Direction direction,
        ...
        System.out.println(result.translationVector());
    }

    private void testCardinal(BoundingBox agentBounds, BlockCollisionView[] collisions, boolean collides) {
        testWalkDirection(agentBounds, List.of(collisions[0]), Direction.NORTH, Vectors.ZERO_INT, collides);
        testWalkDirection(agentBounds, List.of(collisions[1]), Direction.EAST, Vectors.ZERO_INT, collides);
        testWalkDirection(agentBounds, List.of(collisions[2]), Direction.SOUTH, Vectors.ZERO_INT, collides);
        testWalkDirection(agentBounds, List.of(collisions[3]), Direction.WEST, Vectors.ZERO_INT, collides);
    }

    private void testIntercardinal(BoundingBox agentBounds, BlockCollisionView[] collisions, boolean collides) {
        testWalkDirection(agentBounds, List.of(collisions[0], collisions[1]), Direction.NORTHEAST, Vectors.ZERO_INT, collides);
        testWalkDirection(agentBounds, List.of(collisions[1], collisions[2]), Direction.SOUTHEAST, Vectors.ZERO_INT, collides);
        testWalkDirection(agentBounds, List.of(collisions[2], collisions[3]), Direction.SOUTHWEST, Vectors.ZERO_INT, collides);
        testWalkDirection(agentBounds, List.of(collisions[3], collisions[0]), Direction.NORTHWEST, Vectors.ZERO_INT, collides);
    }

    private void testCardinalSameCollision(BoundingBox agentBounds, List<BlockCollisionView> collisions, boolean collides) {
        testWalkDirection(agentBounds, collisions, Direction.NORTH, Vectors.ZERO_INT, collides);
        testWalkDirection(agentBounds, collisions, Direction.EAST, Vectors.ZERO_INT, collides);
        testWalkDirection(agentBounds, collisions, Direction.SOUTH, Vectors.ZERO_INT, collides);
        testWalkDirection(agentBounds, collisions, Direction.WEST, Vectors.ZERO_INT, collides);
    }

    private void testIntercardinalSameCollision(BoundingBox agentBounds, List<BlockCollisionView> collisions, boolean collides) {
        testWalkDirection(agentBounds, collisions, Direction.NORTHEAST, Vectors.ZERO_INT, collides);
        testWalkDirection(agentBounds, collisions, Direction.SOUTHEAST, Vectors.ZERO_INT, collides);
        testWalkDirection(agentBounds, collisions, Direction.SOUTHWEST, Vectors.ZERO_INT, collides);
        testWalkDirection(agentBounds, collisions, Direction.NORTHWEST, Vectors.ZERO_INT, collides);
    }
